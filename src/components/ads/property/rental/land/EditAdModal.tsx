...
function deriveType(category?: string): 'commercial' | 'industrial' | 'agricultural' | '' {
...
export default function RentalLandEditAdModal({ open, onClose, adId, onSaved }: { open: boolean; onClose: () => void; adId: string; onSaved?: () => void }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setBillingPeriod((details.billingPeriod as any) || 'month');
        setSizeValue(String(details?.size?.value || ''));
        setSizeUnit((details?.size?.unit as any) || 'perch');
        setZoning(String(details?.usage?.zoning || ''));
        setAdvanceNum(String(details?.leaseTerms?.advancePayment?.value || ''));
        setAdvanceUnit((details?.leaseTerms?.advancePayment?.unit as any) || 'months');
        setMinLeaseValue(String(details?.leaseTerms?.minLeaseDuration?.value || ''));
        setMinLeaseUnit((details?.leaseTerms?.minLeaseDuration?.unit as any) || 'months');
        setMaxLeaseValue(String(details?.leaseTerms?.maxLeaseDuration?.value || ''));
        setMaxLeaseUnit((details?.leaseTerms?.maxLeaseDuration?.unit as any) || 'months');
        setAccessUtilities(String(details?.extras?.accessUtilities || ''));
        setRestrictionsAmenities(String(details?.extras?.restrictionsAmenities || ''));
  ...
  return (
    <Modal open={open} onClose={onClose} title="Edit Land Rental Ad">
            ...
            <div className="bg-green-50 rounded-xl p-4 md:p-6">
              ...
              <div className="space-y-4">
                ...
                <LocationFields
                  ...
                  placeholder="Galle Road, Commercial Zone"
                  overideZoom={15}
                  editMode
                />
              </div>
